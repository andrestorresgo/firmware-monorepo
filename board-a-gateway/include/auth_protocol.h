...
// Parses string or JSON payload received on factory/actuator/motor into motor state (MOTOR_OFF=0, MOTOR_ON=1, MOTOR_MEDIUM=2)
// Supports "ON", "OFF", "MEDIUM", "on", "off", "medium", and JSON {"state": "ON"}, {"motor_state": 1}, etc.
// Returns true if parsing was successful and state was valid
bool parse_motor_command_payload(const char* payload, size_t len, uint8_t* out_motor_state);

// Constructs and packs a validated ESP-NOW motor command packet (OPCODE_MOTOR_COMMAND) ready for transmission
// Returns the total packed frame length in bytes on success, or negative on validation error
